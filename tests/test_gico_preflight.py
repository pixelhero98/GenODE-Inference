...
def _run_preflight(rows_csv: Path, *extra: str):
    ...
    return preflight_gico_rows(args)


class GicoPreflightTests(unittest.TestCase):
    def test_incomplete_cells_report_missing_support(self) -> None:
        ...
        self.assertEqual(report["support"]["missing_support_cells"][0]["missing_schedule_keys"], ["late_p_2"])
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            _write_rows(
                ...
                [
                    ...
                    _row("ctx", "late_p_2", series_id="series_b", target_t=11),
                ],
            )
    ...
    def test_complete_rows_csv_preserves_header_and_input_order(self) -> None:
        ...
        self.assertEqual([row["scheduler_key"] for row in exported_rows], ["late_p_2", "uniform"])
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            _write_rows(
                ...
                [
                    ...
                    _row("ctx_0", "late_p_2", utility=2.0),
                    _row("ctx_1", "uniform", utility=1.0),
                    _row("ctx_1", "late_p_2", utility=""),
                ],
            )
    ...
    def test_preflight_context_count_accepts_explicit_schema_identity_rows(self) -> None:
        ...
        self.assertEqual(report["status"], "issues_found")
        self.assertEqual(report["context_count_preflight"]["row_error_count"], 0)
    ...
    def test_support_preflight_validator_rejects_schedule_grid_load_error(self) -> None:
        ...
        with self.assertRaisesRegex(ValueError, "schedule_grid_error"):
            validate_gico_support_preflight_report(report)
