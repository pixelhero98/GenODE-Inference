...
class GIPOTests(unittest.TestCase):
    ...
    def test_setting_features_use_fixed_reference_above_seen_nfes(self) -> None:
        ...
        features_20 = setting_features("euler", 20, config=config)
        features_24 = setting_features("euler", 24, config=config)

        self.assertAlmostEqual(
            float(features_20[7]),
            float(np.log1p(20.0) / np.log1p(16.0)),
            places=6,
        )
        self.assertFalse(torch.allclose(features_20, features_24))
    ...
    def test_setting_features_are_invariant_to_query_batch_composition(self) -> None:
        ...
        mixed_batch = torch.stack(
            [
                setting_features("euler", 6, config=config),
                setting_features("euler", 20, config=config),
                setting_features("euler", 24, config=config),
            ]
        )
    ...
    def test_density_mass_for_row_uses_checkpoint_step_for_scoped_summary_grid(self) -> None:
    ...
    def test_student_checkpoint_loader_accepts_only_reference_additive_student(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            setting_dim = setting_feature_dim(SETTING_ENCODER_MODE_CONTINUOUS, config=config)
            ...
            payload = {
                ...
                "setting_dim": setting_dim,
                "mode": SETTING_ENCODER_MODE_CONTINUOUS,
