...
class DiffusionFlowPaperPrepTests(unittest.TestCase):
    ...
    def test_schedule_sets_are_exact(self) -> None:
        self.assertEqual(len(BASELINE_SCHEDULE_KEYS), 12)
        self.assertEqual(
            TRANSFER_SCHEDULE_KEYS,
            (
                "ays_sd15_native",
    ...
    def test_conditional_generation_accepts_summary_schedule_args_without_forecast_datasets(self) -> None:
        ...
        self.assertEqual([case["scheduler_key"] for case in cases], ["uniform", "gico"])
        self.assertEqual(payload["schedule_selection_summary"]["summary_schedule_keys"], ["gico"])
    ...
    def test_conditional_context_embedding_export_batches_full_locked_test_windows(self) -> None:
        class FakeDataset:
            ...
            def __getitem__(self, idx):
                ...
                condition = torch.tensor([float(t0) / 10.0, float(t0) / 20.0])
                return torch.full((2, 3), float(t0)), torch.zeros(3), condition, {"target_t": t0}

        class FakeBackbone(torch.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.cfg = SimpleNamespace(model=SimpleNamespace(hidden_dim=2, cond_dim=2))
                self.batch_shapes = []
                self.condition_shapes = []
                self.grad_enabled = []

            def precompute(self, hist_batch: torch.Tensor, *, cond: torch.Tensor | None = None) -> ConditioningCache:
                ...
                summary = torch.stack([first_value, first_value + 0.5], dim=1)
                return ConditioningCache(
                    ctx_tokens=summary[:, None, :],
                    summary=summary,
                    cond_emb=cond.float(),
                )
        ...
        self.assertEqual(model.backbone.condition_shapes, [(2, 2), (2, 2), (1, 2)])
        ...
        self.assertEqual(embeddings[50], [50.0, 50.5, 5.0, 2.5])
        self.assertEqual(embeddings[40], [40.0, 40.5, 4.0, 2.0])
