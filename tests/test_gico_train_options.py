...
from genode.gico.objectives import CONDITIONAL_PRIMARY_LOB_METRIC_SPECS, FORECAST_METRIC_SPECS
from genode.gico.policy import GICO_PROTOCOL, save_context_embedding_table
...
class GicoTrainOptionsTests(unittest.TestCase):
    def test_parser_defaults_keep_canonical_seen_and_unseen_nfes(self) -> None:
    ...
    def test_omitted_support_requires_canonical_clock_pool_for_reduced_rows(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with self.assertRaisesRegex(ValueError, "Omitted --support_schedule_keys selects the canonical"):
                train_gico(args)

            explicit_summary = train_gico(_trainer_args(root, rows_csv, embeddings_npz))

        self.assertEqual(explicit_summary["support_schedule_keys"], list(SUPPORT_SCHEDULES))
    ...
    def test_legacy_checkpoint_prefixed_context_ids_are_rejected(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with self.assertRaisesRegex(ValueError, "physical context_id values without checkpoint prefixes"):
                train_gico(_trainer_args(root, rows_csv, embeddings_npz, "--seen_target_nfe_values", "5,7"))
    ...
    def test_embedding_overlap_allows_float32_export_noise(self) -> None:
        _assert_embedding_overlap_compatible(
            ...
            label="student_unseen_targets",
        )

    def test_embedding_overlap_rejects_material_difference(self) -> None:
        with self.assertRaisesRegex(ValueError, r"student_unseen_targets context embeddings collide.*max_abs_diff"):
            _assert_embedding_overlap_compatible(
                {"ckpt:ctx": [0.0, 1.0]},
                {"ckpt:ctx": [1.0e-3, 1.0]},
                label="student_unseen_targets",
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            summary = train_gico(
                _trainer_args(
                    ...
                    embeddings_npz,
                    "--support_schedule_keys",
                    "uniform,late_p_2,flowts_power_0p03",
                    "--teacher_density_holdout_schedule_keys",
                    "flowts_power_0p03",
                )
            )
    ...
    def test_train_gico_rejects_utility_rows_with_invalid_reward_metadata(self) -> None:
        cases = [
            ({"gico_reward_protocol": "legacy"}, "gico_reward_protocol"),
            ({"reward_anchor_schedule_key": "late_p_2"}, "reward_anchor_schedule_key"),
    ...
    def test_train_gico_rejects_context_sample_count_above_canonical_cap(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with self.assertRaisesRegex(ValueError, "must not exceed canonical GICO supervision cap"):
                train_gico(
                    _trainer_args(
                        root,
                        rows_csv,
    ...
    def test_train_gico_rejects_zero_rank_pairs_before_teacher_training(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            rows_csv = root / "seen.csv"
            embeddings_npz = root / "ctx.npz"
    ...
    def test_unseen_target_nfe_values_filter_rows_and_summary(self) -> None:
        ...
        self.assertEqual(summary["split_counts"]["student_unseen_targets"]["row_count"], 12)
    ...
    def test_selected_unseen_embedding_overlap_still_rejects_material_difference(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            unseen_embeddings_npz = root / "unseen_ctx.npz"
            _write_rows(seen_csv, target_nfes=CANONICAL_SEEN_NFES)
            _write_rows(unseen_csv, target_nfes=CANONICAL_UNSEEN_NFES)
            _write_embeddings(seen_embeddings_npz)
            save_context_embedding_table(
                unseen_embeddings_npz,
            ...
            with self.assertRaisesRegex(ValueError, r"student_unseen_targets context embeddings collide.*ctx_0"):
                train_gico(
                    _trainer_args(
                        root,
                        seen_csv,
    ...
    def test_unseen_support_schedules_must_match_seen_support_schedules(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            embeddings_npz = root / "ctx.npz"
            _write_rows(seen_csv, target_nfes=CANONICAL_SEEN_NFES)
            _write_rows(unseen_csv, target_nfes=CANONICAL_UNSEEN_NFES, schedules=("uniform", "ays_sd15_native"))
            _write_embeddings(embeddings_npz)

            with self.assertRaisesRegex(ValueError, r"missing=\['late_p_2'\], extra=\['ays_sd15_native'\]"):
                train_gico(
                    _trainer_args(
                        root,
                        seen_csv,
