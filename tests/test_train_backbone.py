...
def _args(**overrides):
    base = {
        ...
        "checkpoint_export_mode": "best_validation_within_budget",
        "training_state_out": "",
...
class TrainBackboneTests(unittest.TestCase):
    ...
    def test_train_backbone_supports_validation_best_budget_artifacts(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            matrix_root = Path(tmpdir) / "matrix"
            fake_model = _FakeModel()
            ...
            for steps, selected_step in ((1, 1), (2, 2)):
                ...
                self.assertEqual(metadata["effective_train_steps"], selected_step)
                self.assertEqual(metadata["checkpoint_export_protocol"], "best_validation_state_within_budget")
    ...
    def test_train_backbone_rejects_resume_mode_mismatch(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            matrix_root = Path(tmpdir) / "matrix"
            state_path = Path(tmpdir) / "restart.pt"
            fake_model = _FakeModel()
