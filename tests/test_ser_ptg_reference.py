...
class SerPtgReferenceTests(unittest.TestCase):
    ...
    def test_forecast_train_tuning_is_capped_by_context_sample_count(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmpdir, mock.patch.object(ser, "load_forecast_checkpoint_splits", return_value=checkpoint), mock.patch.object(
            ...
            args = build_argparser().parse_args(
                [
                    ...
                    str(TRAIN_TUNING_CONTEXT_SAMPLE_COUNT),
                    "--out_dir",
                    tmpdir,
                    "--device",
                    "cpu",
        ...
        self.assertEqual(len(captured[0]), TRAIN_TUNING_CONTEXT_SAMPLE_COUNT)
        ...
        self.assertEqual(prediction["selected_examples"], TRAIN_TUNING_CONTEXT_SAMPLE_COUNT)
        self.assertEqual(prediction["selected_examples_cap"], TRAIN_TUNING_CONTEXT_SAMPLE_COUNT)
        ...
        self.assertGreater(prediction["uncapped_candidate_examples"], TRAIN_TUNING_CONTEXT_SAMPLE_COUNT)
        self.assertEqual(summary["selected_examples_cap"], TRAIN_TUNING_CONTEXT_SAMPLE_COUNT)
        self.assertEqual(summary["selected_examples_cap_source"], "context_sample_count")
        self.assertEqual(summary["local_defect_trace_protocol"], "otflow_midpoint_local_defect_proxy")
        self.assertEqual(summary["oracle_local_error_semantics"], "local_defect_proxy_not_teacher_oracle")
        self.assertEqual(summary["scenario_key"], "traffic_hourly")
        self.assertEqual(summary["scheduler_key"], ser.SER_PTG_SCHEDULE_KEY)
        self.assertNotIn("dataset", summary)
        self.assertNotIn("schedule_key", summary)
        self.assertNotIn("trace_variant", summary)
    ...
    def test_conditional_generation_over_selection_is_capped(self) -> None:
        ...
        self.assertEqual(requested_windows, [TRAIN_TUNING_CONTEXT_SAMPLE_COUNT])
        self.assertEqual(len(captured[0]), TRAIN_TUNING_CONTEXT_SAMPLE_COUNT)
        prediction = summary["predictions"][0]
        self.assertEqual(prediction["selected_examples"], TRAIN_TUNING_CONTEXT_SAMPLE_COUNT)
        self.assertEqual(prediction["selected_examples_cap"], TRAIN_TUNING_CONTEXT_SAMPLE_COUNT)
        ...
        self.assertEqual(
            ...
            TRAIN_TUNING_CONTEXT_SAMPLE_COUNT + 64,
        )
