...
from genode.gico.train_gico import build_argparser as build_train_argparser
...
class GICOCanonicalTests(unittest.TestCase):
    ...
    def test_teacher_diagnostics_candidate_schema_uses_checkpoint_scope(self) -> None:
        diagnostics = gico_teacher_diagnostics(
            _FixedScoreTeacher(),
            [],
            context_embeddings={},
            schedule_grids=None,
    ...
    def test_teacher_weighted_targets_support_elite_and_blend_modes(self) -> None:
        reference = uniform_reference_grid(64)
        schedules = ("uniform", "late_p_2", "flowts_power_0p03", "ays_sd15_native")
        scores = {
            "uniform": 0.0,
            "late_p_2": 3.0,
            "flowts_power_0p03": 1.0,
            "ays_sd15_native": 2.0,
        }
        rows = [
            {
    ...
    def test_teacher_weighted_targets_reject_mixed_cell_scalarization(self) -> None:
        ...
        with self.assertRaisesRegex(ValueError, "identical teacher metric masks"):
            build_teacher_weighted_density_targets(
                ...
                reference_time_grid=reference,
                density_normalizer=normalizer,
                supervision_schedule_keys=("uniform", "late_p_2"),
                temperature=1.0,
                device="cpu",
            )
    ...
    def test_teacher_training_summary_keeps_global_pair_count_under_minibatching(self) -> None:
        ...
        rows = []
        for idx in range(70):
            for schedule, score in (("uniform", 0.0), ("late_p_2", 1.0)):
                rows.append(
                    {
                        "dataset": "lobster_synthetic",
                        "split_phase": "train_tuning",
                        ...
                        "context_embedding_id": f"ctx_{idx}",
        ...
        teacher = build_gico_teacher_model(
            architecture=ARCHITECTURE_DENSITY_FORM_TRANSFORMER,
    ...
    def test_models_do_not_expose_series_conditioning(self) -> None:
        ...
        student = GICODensityQueryStudentTransformer(
        ...
        teacher = GICODensityFormTeacherTransformer(
            setting_dim=int(setting_batch.shape[1]),
            density_dim=64,
            context_dim=2,
            hidden_dim=16,
        ...
        for callable_ in (student.logits, student.density_mass, teacher.forward):
            signature = inspect.signature(callable_)
            self.assertNotIn("rows", signature.parameters)
            self.assertNotIn("series_index_batch", signature.parameters)
    ...
    def test_non_additive_conditioning_style_is_rejected(self) -> None:
        ...
        with self.assertRaisesRegex(ValueError, "conditioning_style|additive_mlp"):
            GICODensityQueryStudentTransformer(**model_kwargs)
        with self.assertRaisesRegex(ValueError, "conditioning_style|additive_mlp"):
            GICODensityFormTeacherTransformer(**model_kwargs)
        with self.assertRaisesRegex(ValueError, "conditioning_style|additive_mlp"):
            build_gico_student_model(
    ...
    def test_trainer_argparser_exposes_only_canonical_conditioning(self) -> None:
        ...
        self.assertNotIn("--gico_" + "conditioning_style", options)
        self.assertNotIn("--gico_teacher_" + "conditioning_style", options)
        self.assertNotIn("--gico_student_" + "conditioning_style", options)
    ...
    def test_student_checkpoint_loader_accepts_only_canonical_additive_student(self) -> None:
        from genode.gico.report_locked_test import _load_student_checkpoint
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            checkpoint_path = root / "gico_student.pt"
            payload = {
                ...
                "teacher_utility_weights": {"u_comp_uniform": 1.0},
                "locked_test_used_for_selection": False,
            }
            torch.save(payload, checkpoint_path)

            with mock.patch("genode.gico.report_locked_test.torch.load", return_value=payload) as mocked_load:
            ...
            retired_payload = {**payload, "series_index_map": {"series_0": 0}}
            torch.save(retired_payload, retired_path)
            with self.assertRaisesRegex(ValueError, "retired metadata keys"):
                _load_student_checkpoint(retired_path)

            old_version_path = root / "old_version_student.pt"
            old_version_payload = {**payload, "model_payload_version": MODEL_PAYLOAD_VERSION - 1}
            torch.save(old_version_payload, old_version_path)
            with self.assertRaisesRegex(ValueError, "unsupported model_payload_version"):
                _load_student_checkpoint(old_version_path)
    ...
    def test_locked_report_conditioning_metadata_is_canonical(self) -> None:
        from genode.gico.report_locked_test import _conditioning_metadata_for_summary
