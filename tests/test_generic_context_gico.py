...
class GenericContextGicoTests(unittest.TestCase):
    ...
    def test_forecast_unseen_rows_are_reward_materialized_before_student_distillation(self) -> None:
        def write_rows(path: Path, target_nfes: tuple[int, ...]) -> None:
            schedules = {
                "uniform": (2.0, 2.0),
                "late_p_2": (1.0, 1.0),
                "flowts_power_0p03": (1.5, 1.5),
                "ays_sd15_native": (1.4, 1.4),
            }
            fields = [
                "benchmark_family",
                "dataset",
                            ...
                            writer.writerow(
                                {
                                    ...
                                    "context_embedding_id": f"ctx_{ctx_idx}",
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            args = SimpleNamespace(
                ...
                support_schedule_keys="uniform,late_p_2,flowts_power_0p03,ays_sd15_native",
                extra_late_p_values="",
                ...
                teacher_density_holdout_schedule_keys="flowts_power_0p03,ays_sd15_native",
                ...
                student_unseen_target_rows_csv=str(unseen_rows),
        ...
        self.assertFalse(summary["student_objective_settings"]["student_teacher_score_include_unseen_targets"])
        ...
        unseen_summary = summary["student_training"]["student_unseen_target_summary"]
        self.assertTrue(unseen_summary["unseen_target_distillation_used"])
        self.assertEqual(unseen_summary["unseen_target_nfes"], [6, 10, 14, 20])
        self.assertEqual(unseen_summary["student_target_mixture_mode"], "full")
    ...
    def test_conditional_context_rows_use_physical_ids_and_checkpoint_scoped_embeddings(self) -> None:
        ...
        self.assertEqual(rows[0]["gico_reward_protocol"], GICO_PROTOCOL)
    ...
    def test_context_embedding_id_is_canonical_lookup_key(self) -> None:
        ...
        with self.assertRaisesRegex(ValueError, "explicit context_embedding_id"):
            context_embedding_id_from_row({"context_id": "logical-only"})
    ...
    def test_support_group_counts_are_checkpoint_aware_for_physical_context_ids(self) -> None:
        ...
        _validate_support_group_counts(rows, ["uniform", "late_p_2"])
    ...
    def test_locked_reporter_coverage_uses_logical_seed_when_sampling_seed_is_offset(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            training_summary = tmp / "gico_training_summary.json"
            ...
            args = SimpleNamespace(
                gico_student_checkpoint=str(tmp / "gico_student.pt"),
    ...
    def test_locked_reporter_output_uses_logical_seed_with_offset_uniform_context_match(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with (
                ...
                mock.patch(
                    "genode.gico.report_locked_test.load_context_embedding_table",
                    return_value={"ckpt:ctx-4": [0.0, 1.0]},
                ),
                mock.patch(
                    "genode.gico.report_locked_test.load_forecast_checkpoint_splits",
                    return_value={
                        "model": mock.Mock(),
                        "cfg": SimpleNamespace(),
                ...
                mock.patch(
                    "genode.gico.report_locked_test.predict_gico_density_many",
                    return_value=[
                        {
                            "time_grid": [0.0, 1.0],
                            "density_mass": [1.0],
                ...
                mock.patch(
                    "genode.gico.report_locked_test.evaluate_forecast_schedule",
                    return_value={"forecast_crps": 1.0, "forecast_mase": 1.0, "forecast_mse": 1.0},
                ),
            ):
                summary = report_locked_test.report_gico_locked_test(args)

            self.assertEqual(summary["artifact"], "gico_locked_test_report")
    ...
    def test_family_aware_comparison_summary_handles_molecule_metrics(self) -> None:
        ...
        self.assertEqual(ranking["metric_rankings"]["molecule_kabsch_rmsd_3d"], ["gico", "uniform"])
    ...
    def test_conditional_comparison_summary_uses_primary_metrics_and_tstr_high_direction(self) -> None:
        ...
        self.assertEqual(ranking["metric_rankings"]["temporal_tstr_f1"], ["gico", "uniform"])
    ...
    def test_full_pipeline_dry_run_accepts_molecule_scenario_without_manifest(self) -> None:
        ...
        self.assertTrue(any("genode.gico.report_locked_test" in " ".join(command) for command in commands))
    ...
    def test_full_pipeline_zero_shot_stage_does_not_use_unseen_selection_rows(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            args = full_pipeline.build_argparser().parse_args(
                [
                    ...
                    str(Path(tmpdir) / "run"),
                    "--stages",
                    "gico_student_seen_only_zero_shot,gico_student_seen_plus_unseen_targets",
                    "--dry_run",
                ]
            )
            summary = full_pipeline.run_full_pipeline(args)
        by_stage = {stage["stage"]: [" ".join(command) for command in stage["commands"]] for stage in summary["stages"]}
        zero_shot = " ".join(by_stage["gico_student_seen_only_zero_shot"])
        unseen = " ".join(by_stage["gico_student_seen_plus_unseen_targets"])
        ...
        self.assertIn("--student_unseen_target_rows_csv", unseen)
        self.assertNotIn("--student_teacher_score_include_unseen_targets", unseen)
        self.assertIn("--teacher_utility_weights", unseen)
        protocol = full_pipeline._protocol_payload(args)
        self.assertEqual(protocol["gico_teacher_steps"], 500)
        self.assertEqual(protocol["gico_student_steps"], 500)
    ...
    def test_full_pipeline_gico_step_budgets_are_protocolized(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
        ...
        self.assertEqual(base_protocol["gico_teacher_steps"], 500)
        self.assertEqual(base_protocol["gico_student_steps"], 500)
        self.assertEqual(changed_protocol["gico_teacher_steps"], 501)
        self.assertEqual(changed_protocol["gico_student_steps"], 503)
        self.assertNotEqual(full_pipeline._json_hash(base_protocol), full_pipeline._json_hash(changed_protocol))

    def _dry_run_gico_commands_for_scenario(self, scenario_key: str) -> str:
        with tempfile.TemporaryDirectory() as tmpdir:
            args = full_pipeline.build_argparser().parse_args(
                [
                    "--scenario_key",
    ...
    def test_full_pipeline_forecast_uses_forecast_teacher_target_vector(self) -> None:
        commands = self._dry_run_gico_commands_for_scenario("solar_energy_10m")
    ...
    def test_full_pipeline_conditional_uses_conditional_teacher_target_vector(self) -> None:
        commands = self._dry_run_gico_commands_for_scenario("lobster_synthetic")
    ...
    def test_full_pipeline_ecg_uses_two_metric_teacher_target_vector(self) -> None:
        commands = self._dry_run_gico_commands_for_scenario("long_term_st")
    ...
    def test_full_pipeline_molecule_uses_molecule_teacher_target_vector(self) -> None:
        commands = self._dry_run_gico_commands_for_scenario("molecule_3d_set1")
    ...
    def test_gico_ablation_preset_covers_main_and_appendix_arms(self) -> None:
        arms = gico_ablation_arms(GICO_ABLATION_PRESET_PAPER_MAIN_PLUS_APPENDIX)
    ...
    def test_full_pipeline_ablation_first_default_dry_run_grid_is_clean(self) -> None:
        ...
        self.assertEqual(
            ...
            [
                ...
                "gico_ablation_locked_test_reports",
            ],
        )
        self.assertFalse(any(stage["stage"] == "backbone_training" for stage in summary["stages"]))
        self.assertFalse(any(stage["stage"].startswith("gico_student_") for stage in summary["stages"]))
        by_stage = {stage["stage"]: stage["commands"] for stage in summary["stages"]}
        self.assertEqual(len(by_stage["gico_ablation_students"]), 16)
        self.assertEqual(len(by_stage["gico_ablation_locked_test_reports"]), 160)
        for stage_name in ("schedule_rows_seen", "schedule_rows_unseen"):
            ...
            for command in by_stage[stage_name]:
                ...
                self.assertIn("--baseline_scheduler_names", command)
    ...
    def test_full_pipeline_does_not_apply_supervision_cap_to_locked_test_rows(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            args = full_pipeline.build_argparser().parse_args(
                [
                    ...
                    str(Path(tmpdir) / "run"),
                    "--ablation_first",
                    "--gico_supervision_context_sample_count",
                    "7",
                    "--dry_run",
                ]
            )
        ...
        self.assertTrue(all("--context_sample_count 7" in command for command in gico_commands))
        report_commands = [" ".join(command) for command in by_stage["gico_ablation_locked_test_reports"]]
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            args = full_pipeline.build_argparser().parse_args(
                [
                    ...
                    str(Path(tmpdir) / "run"),
                    "--stages",
                    "data_prep,schedule_rows_seen,schedule_rows_unseen,gico_ablation_students,gico_ablation_locked_test_reports",
                    "--schedule_keys",
                    "uniform,late_p_2",
                    "--checkpoint_steps",
                    "4000",
    ...
    def test_full_pipeline_ablation_first_routes_arm_knobs_and_locked_reports(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            summary = full_pipeline.run_full_pipeline(args)
        by_stage = {stage["stage"]: [" ".join(command) for command in stage["commands"]] for stage in summary["stages"]}
        train_commands = by_stage["gico_ablation_students"]
        report_commands = by_stage["gico_ablation_locked_test_reports"]
        for arm in gico_ablation_arms(GICO_ABLATION_PRESET_PAPER_MAIN_PLUS_APPENDIX):
    ...
    def test_full_pipeline_default_ablation_first_run_root_is_namespaced(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertTrue((expected_root / "gico_ablation_students_manifest.json").exists())
            ...
            self.assertIn("gico_ablations", summary["run_root"])
    ...
    def test_full_pipeline_failed_ablation_stage_marks_ablation_manifest_failed(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            failing_stage = full_pipeline.StageCommand(
                full_pipeline.GICO_ABLATION_STUDENT_STAGE,
                [[full_pipeline.sys.executable, "-c", "import sys; sys.exit(3)"]],
                "gico_ablation_students_manifest.json",
            )
            with (
                mock.patch.object(full_pipeline, "_validate_inputs_preflight", return_value={"status": "complete"}),
