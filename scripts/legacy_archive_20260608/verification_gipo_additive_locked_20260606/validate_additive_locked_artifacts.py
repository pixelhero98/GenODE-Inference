...
def validate_panel(root: Path, panel: str, expected_nfes: Sequence[int]) -> dict[str, Any]:
    ...
    manifest = {
        "artifact": "gipo_additive_locked_panel_artifact_manifest",
        "panel": panel,
        ...
        "multi_nfe_group_diagnostics": {
            "train_supervision": _group_diagnostics(train_rows, seed_kind="logical"),
            "locked_teacher_support": _group_diagnostics(support_rows, seed_kind="evaluation"),
        },
    }
...
def validate_artifacts(root: Path, panels: Iterable[str]) -> dict[str, Any]:
    ...
    payload = {
        "artifact": "gipo_additive_locked_artifact_validation",
        "root": str(root),
        "validation_passed": not issues,
        "issues": issues,
        "panels": manifests,
