...
def _first_student_comparison(cell: Mapping[str, Any]) -> Mapping[str, Any]:
    ...
    return dict(comparisons[0]) if comparisons else {}
...
def _markdown(root: Path, policy_run_id: str, policy_id: str, manifest: Mapping[str, Any], report: Mapping[str, Any], overall: Mapping[str, Any], by_nfe: Mapping[str, Mapping[str, Any]]) -> str:
    lines = [
        "# Density-Diversity Unseen-NFE Locked-Test Evaluation",
        "",
        f"Output root: `{root}`",
        "",
        f"Policy: `{policy_run_id}` (`{policy_id}`), selected by calibration diagnostics before this unseen-NFE evaluation.",
    ...
    lines.extend(["", "Detailed cell-level gains are in `summary/unseen_nfe_cell_gains.csv`.", ""])
    return "\n".join(lines)
...
def main() -> None:
    root = Path(
        os.environ.get(
            "GENODE_UNSEEN_ROOT",
            "/scratch/b35z/pixelhero.b35z/genode/outputs/verification_density_diversity_20260603/unseen_nfe_locked_winner_6_10_14_16",
        )
    )
    ...
    training = _read_json(root.parent / "policy_runs" / policy_run_id / "gipo_training_summary.json")
    ...
    payload = {
        "artifact": "density_diversity_unseen_nfe_locked_winner_summary",
    ...
    (summary_dir / "final_unseen_nfe_report.md").write_text(
        _markdown(root, policy_run_id, str(training.get("policy_id")), manifest, report, overall, by_nfe),
        encoding="utf-8",
    )
    print(json.dumps({"summary_dir": str(summary_dir), "policy_run_id": policy_run_id, "cell_count": len(rows)}, sort_keys=True))
