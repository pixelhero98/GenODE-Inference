...
cp "${SCRIPT_DIR}/05_merge_calibration.sbatch" "${GENODE_VERIFICATION_ROOT}/jobs/"
cp "${SCRIPT_DIR}/collect_summary.py" "${GENODE_VERIFICATION_ROOT}/jobs/"
cp "${SCRIPT_DIR}/merge_calibration_inputs.py" "${GENODE_VERIFICATION_ROOT}/jobs/"
...
merge_job=$(sbatch --parsable --partition="${GENODE_CPU_PARTITION}" "${GENODE_VERIFICATION_ROOT}/jobs/05_merge_calibration.sbatch")
...
python3 - "${GENODE_VERIFICATION_ROOT}" "${merge_job}" "${summary_job}" <<'PY'
import json
import os
import sys
...
payload = {
    "resume_mode": "after_calibration_merge",
    "merge_job": sys.argv[2],
