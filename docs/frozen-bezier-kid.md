...
interface with global utility surrogate and policy conditioning. The [built-in evaluator](evaluators.md) executes the pinned generator and paired KID measurements.
...
Utility surrogate ranking/regression, regret-based utility surrogate selection, policy distillation
plus utility surrogate-score chasing, and held-out policy selection use
the shared implementations. Held-out utility is paired KID improvement for this
objective. Inference uses only the frozen policy and the composed clock.
...
Deterministic checkpoint selection uses frozen-utility surrogate utility subject to the 15% validation-KL allowance; stochastic selection uses expected calibrated frozen-utility surrogate utility subject to its separate 20% full-distribution-KL allowance. See [policy selection](policy-selection.md).
