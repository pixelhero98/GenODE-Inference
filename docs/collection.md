...
For 256 contexts, the 25 density assignments receive ten contexts each and six seeded random densities receive one extra. Sampling is stratified round-robin over shuffled source strata. The default collection seeds are `(0, 1)`; they are generation repeats, separate from the collection sampling seed and policy RNG. The selected contexts, assignments and split membership are shared across NFEs. Every context's repeats and anchors stay together. If fewer contexts are available, the resolved count is recorded rather than silently adding solves; at least 50 are required to populate both splits across the pool.
