...
- Forecast entries also specify `dataset_root` and `time_feature_mode`. The checkpoint determines history and prediction horizons. Each case specifies `backbone` (the map key), `example_idx` in the validation dataset, `reference_id`, `target_sha256`, `collection_batch_size: 1`, and complete consecutive `sample_seed_values`. Use the manifest's physical member seeds for collection cases and retain them for reporting; a panel's logical seed alone cannot replay a later example.
