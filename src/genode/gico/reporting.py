"""Explicit terminal-utility reports; never called by policy fitting or selection."""
...
REPORT_PROTOCOL = "paired_terminal_utility_report"
...
def measured_utility(
    rows: list[dict], evidence: Evidence, candidate: PolicyCandidate, *, clock_replicates: int
) -> dict:
...
def evaluate_policy_report(
    model, conditioning, kind, step, coefficient, evaluator: Callable, evidence, clock_replicates
):
    ...
    try:
        with torch.random.fork_rng(devices=devices), torch.inference_mode():
            ...
            candidate = PolicyCandidate(snapshot, copy.deepcopy(conditioning), kind, step, coefficient, identity)
