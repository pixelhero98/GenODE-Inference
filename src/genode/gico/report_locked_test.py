...
def _validate_density_bin_count(payload: Mapping[str, Any], density_meta: Mapping[str, Any], *, role: str) -> None:
    expected = REQUIRED_GICO_DENSITY_BIN_COUNT
    ...
    if density_dim != expected or reference_bin_count != expected or len(reference_grid) != expected + 1:
        raise ValueError(
            f"GICO {role} checkpoints require {expected} density bins; "
...
def _validate_context_rows(
    ...
    *,
    ...
    if bad:
        raise ValueError(f"GICO reporter expected source split_phase={requested!r}; found {bad}.")
...
def _load_student_checkpoint(
    ...
    payload = require_current_gico_checkpoint_payload(
        ...
        label=f"GICO student checkpoint {checkpoint_path}",
    )
    if str(payload.get("protocol", "")) != GICO_PROTOCOL:
        raise ValueError(f"Unsupported GICO student protocol {payload.get('protocol')!r}; expected {GICO_PROTOCOL!r}.")
    if str(payload.get("context_embedding_protocol", "")) != FROZEN_BACKBONE_POLICY_CONTEXT_PROTOCOL:
        raise ValueError(
            "GICO student checkpoint must use the frozen-backbone policy context protocol "
            f"{FROZEN_BACKBONE_POLICY_CONTEXT_PROTOCOL!r}."
        )
    if str(payload.get("student_policy_type", "")) != "continuous_density":
        raise ValueError("GICO locked reporter only accepts continuous_density student checkpoints.")
    if bool(payload.get("locked_test_used_for_selection", False)):
        raise ValueError("GICO student checkpoint indicates locked_test was used for selection.")
    density_meta = dict(payload.get("density_representation", {}))
    if str(density_meta.get("density_protocol", "")) != "density_mass":
        raise ValueError("GICO student checkpoint is missing density_mass metadata.")
    ...
    if int(payload["setting_dim"]) != int(expected_setting_dim):
        raise ValueError(
            f"GICO student checkpoint setting_dim={payload['setting_dim']} does not match "
    ...
    if student_architecture != ARCHITECTURE_DENSITY_QUERY_TRANSFORMER:
        raise ValueError(
            f"GICO student checkpoints must use {ARCHITECTURE_DENSITY_QUERY_TRANSFORMER!r}; got {student_architecture!r}."
        )
    ...
    validate_gico_density_token_attention(student_model_config, require_present=True)
    validate_gico_attention_heads(int(student_model_config.get("attention_heads", -1)))
...
def _conditioning_metadata_for_summary(
    ...
    if len(candidates) > 1:
        raise ValueError(f"GICO metadata contains inconsistent conditioning_style values: {sorted(candidates)}")
    if not candidates:
        return {}
...
def _benchmark_family_from_rows(rows: Sequence[Mapping[str, Any]], requested: str = "") -> str:
    ...
    if not families:
        ...
        if has_generic_context_schema:
            raise ValueError("Generic GICO context rows require benchmark_family for locked-test reporting.")
        return FORECAST_FAMILY
    if len(families) != 1:
        raise ValueError(f"GICO reporter requires one benchmark_family per report; found {sorted(families)}.")
...
def _output_prefix(split_phase: str, selection_mode: str, report_label: str = "") -> str:
    if str(split_phase) == LOCKED_TEST_PHASE and str(selection_mode) == SELECTION_MODE_REPORTING:
        return "locked_test_gico"
    label = str(report_label).strip() or str(split_phase).strip() or "gico"
...
def _molecule_member_from_row(row: Mapping[str, Any]) -> tuple[str, str]:
    ...
    stratum = str(row.get("stratum") or row.get("axis_stratum") or "").strip()
    if not member_key or not stratum:
        raise ValueError("Molecule GICO reporting rows require member_key/axis_member and stratum/axis_stratum.")
...
def _attach_uniform_rewards_to_gico_row(
    row: Mapping[str, Any],
    *,
    ...
    if uniform_row is None:
        raise ValueError(
            "Locked-test GICO reporting requires a matched uniform row for every "
    ...
    reward_columns = uniform_anchored_objective_columns(
        {**out, "scheduler_key": GICO_SCHEDULE_KEY},
    ...
    out["gico_reward_protocol"] = GICO_PROTOCOL
...
def report_gico_locked_test(args: argparse.Namespace) -> dict[str, Any]:
    ...
    training_summary = require_current_gico_checkpoint_payload(
        json.loads(resolve_project_path(str(args.training_summary)).read_text(encoding="utf-8")),
        label="GICO training summary",
    )
    if str(training_summary.get("protocol", "")) != GICO_PROTOCOL:
        raise ValueError("training_summary protocol does not match continuous-density GICO.")
    ...
    if required_checkpoint_selection_mode:
        ...
        if actual_selection_mode != required_checkpoint_selection_mode:
            raise ValueError(
                f"GICO reporter requires teacher_checkpoint_selection_mode={required_checkpoint_selection_mode!r}; "
    ...
    if not context_rows_arg.strip():
        raise ValueError("GICO reporter requires --context_rows.")
    if not embeddings_arg.strip():
        raise ValueError("GICO reporter requires --context_embeddings_npz.")
    ...
    with ProgressBar(len(representatives), f"GICO {split_phase}") as progress:
        for row_idx, row in enumerate(representatives):
            ...
            if benchmark_family == FORECAST_FAMILY:
                ...
                metrics = evaluate_forecast_schedule(
                    ...
                    seed=int(eval_seed),
                    scheduler_key=GICO_SCHEDULE_KEY,
                    dataset_key=dataset,
                ...
                metrics = run_fixed_schedule_variant(
                    ...
                    eval_windows=1,
                    grid_spec={
                        "grid_name": GICO_SCHEDULE_KEY,
                        "grid_kind": "gico_density_time_grid",
                        "selection_group": GICO_SCHEDULE_KEY,
            ...
            selected_row = {
                ...
                "gico_evaluation_seed": int(eval_seed),
            ...
            selected_row = _attach_uniform_rewards_to_gico_row(
                selected_row,
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    summary = report_gico_locked_test(args)
