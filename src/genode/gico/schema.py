...
def reject_retired_evaluation_keys(value: Any, *, source: str) -> None:
    ...
    if found:
        raise ValueError(
            ...
            "use scenario_key, scheduler_key, gico_step_budget, checkpoint_step, "
...
def evaluation_row_signature(
    *,
    ...
    scheduler_key: str,
    checkpoint_id: str,
    schedule_parameters: Mapping[str, Any] | str | None = None,
) -> str:
    """Build the stable identity for one evaluation result row."""

    normalized_parameters = parse_schedule_parameters(
        scheduler_key,
        schedule_parameters,
    )
    return "|".join(
        [
            ...
            str(solver_key),
            str(scheduler_key),
            schedule_parameters_json(scheduler_key, normalized_parameters),
            str(checkpoint_id),
        ]
    )
