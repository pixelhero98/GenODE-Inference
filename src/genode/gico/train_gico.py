...
from genode.gico.density_representation import density_metadata, reference_grid_hash, uniform_reference_grid
...
from genode.gico.preflight import (
    teacher_metric_target_coverage,
)
from genode.gico.schedule_grids import load_schedule_summary_grids, validate_schedule_grid_coverage
...
def _infer_single_benchmark_family(rows: Sequence[Mapping[str, Any]]) -> str:
    ...
    if len(families) != 1:
        raise ValueError(f"GICO training rows must contain exactly one benchmark_family; found {sorted(families)}.")
    family = next(iter(families))
    if family not in {SCENARIO_FAMILY_FORECAST, SCENARIO_FAMILY_CONDITIONAL_GENERATION, SCENARIO_FAMILY_MOLECULE}:
        raise ValueError(f"Unsupported benchmark_family for GICO teacher target selection: {family!r}.")
    return family
...
def _infer_single_dataset_key(rows: Sequence[Mapping[str, Any]]) -> str:
    ...
    if len(datasets) > 1:
        raise ValueError(f"GICO training rows must contain exactly one dataset/scenario key; found {sorted(datasets)}.")
...
def _needs_forecast_uniform_rewards(rows: Sequence[Mapping[str, Any]], target_keys: Sequence[str]) -> bool:
    ...
    if not any(key in reward_keys for key in target_keys):
        return False
    return bool(
        _rows_are_forecast_family(rows)
        and _rows_have_forecast_metrics(rows)
        and _missing_target_value_keys(rows, target_keys)
    )
...
def _select_context_density_regret_step(
    *,
    ...
    selection = select_weighted_normalized_regret_checkpoint(
        [
            {"step": int(step), "diagnostics": dict(entry.get("diagnostics", {}) or {})}
            for step, entry in sorted(context_history.items())
        ],
        required_split_names=required_splits,
        component_weights=split_weights,
    )
...
def _observed_support(
    ...
    *,
    ...
    return validate_gico_support_schedule_keys(canonical_order + extras)
    ...
    *,
    ...
    for key, item in dict(coverage.get("metrics", {}) or {}).items():
        ...
        if valid_count < int(min_valid_rows) or fraction + 1e-12 < float(min_coverage_fraction):
            failures.append(
                f"{key}: valid_rows={valid_count}/{int(coverage.get('row_count', 0))}, coverage={fraction:.6g}"
            )
    if failures:
        raise ValueError(
...
def _teacher_selection_weight_summary(
    ...
    *,
    ...
    return {
        "protocol": GICO_TEACHER_SELECTION_WEIGHT_PROTOCOL,
...
def _validate_support_group_counts(rows: Sequence[Mapping[str, Any]], support_schedule_keys: Sequence[str]) -> None:
    ...
    for row in rows:
        ...
        counts = grouped.setdefault(tuple(context_pair_key(row, pair_on_seed=True)), dict.fromkeys(support_keys, 0))
    ...
    if bad:
        ...
        raise ValueError(
            "GICO supervision requires exactly one row for every support schedule in each "
...
def _validate_rank_pair_preflight(rank_pair_preflight: Mapping[str, Mapping[str, Any]]) -> None:
    ...
    if failures:
        raise ValueError(
            "GICO rank_pair_preflight failed before teacher training; "
...
def _validate_context_embedding_checkpoint_scope(rows: Sequence[Mapping[str, Any]], *, label: str) -> None:
    ...
    if mismatches:
        raise ValueError(
            f"{label} requires checkpoint-scoped context_embedding_id values; first mismatches: {mismatches[:8]}."
        )
    ...
    *,
    ...
    return {
        ...
        "available_physical_context_count": int(
            len({context_id for values in available_by_checkpoint.values() for context_id in values})
        ),
...
def _assert_embedding_overlap_compatible(
    ...
    *,
    ...
    if bad:
        examples = [
            f"{key} (shape mismatch)" if diff is None else f"{key} (max_abs_diff={diff:.3g})" for key, diff in bad[:8]
        ]
        raise ValueError(
...
def _validate_train_tuning_rows(rows: Sequence[Mapping[str, Any]], *, label: str) -> None:
    locked_rows = [
        row for row in rows if _source_split_phase(row) == "locked_test" or _raw_split_phase(row) == "locked_test"
    ]
    if locked_rows:
    ...
    *,
    ...
    for row_idx, row in enumerate(rows):
        ...
        if present_target_keys:
            protocol = str(row.get("gico_reward_protocol", "")).strip()
            if protocol != GICO_PROTOCOL:
                raise ValueError(
                    f"{label} row {row_idx} has utility targets but gico_reward_protocol={protocol!r}; "
                    f"expected {GICO_PROTOCOL!r}."
                )
            ...
            for key, value in weight_payload.items():
                try:
                    ...
                    raise ValueError(
                        f"{label} row {row_idx} reward_metric_weights_json values must be numeric."
                    ) from exc
...
def build_argparser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        default="",
        help=(
            "Comma-separated reference-clock supervision keys. When omitted, the canonical 23-clock pool "
            "plus any --extra_late_p_values pairs is required. Reduced pools must be explicit."
        ),
    )
    parser.add_argument(
        "--extra_late_p_values",
        default="",
        help="Comma-separated analytic late-p supervision augmentations in [1.5, 8]; each adds its reversed clock.",
    )
    ...
    parser.add_argument(
        ...
        default=",".join(str(value) for value in CANONICAL_UNSEEN_NFES),
        help="Comma-separated unseen-target NFEs selected from --student_unseen_target_rows_csv.",
    )
    parser.add_argument("--student_unseen_target_weight", type=float, default=CANONICAL_UNSEEN_TARGET_WEIGHT)
...
def train_gico(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    validate_gico_attention_heads(int(args.transformer_heads))
    ...
    if locked_rows:
        raise ValueError(
            f"GICO training refuses locked_test rows in rows_csv; found {len(locked_rows)} locked-test rows."
        )
    ...
    if observed_source_phases and observed_source_phases != ["train_tuning"]:
        raise ValueError(
            f"GICO training rows_csv must contain only train_tuning source rows; found {observed_source_phases}."
        )
    _validate_unique_schedule_rows(rows, label="GICO training rows_csv")
    _validate_context_embedding_checkpoint_scope(rows, label="GICO training rows_csv")
    validate_teacher_objective_hyperparameters(
    ...
    extra_late_p_values = parse_extra_late_p_values(str(args.extra_late_p_values))
    requested_extra_late_p_keys = tuple(
        key
        for power in extra_late_p_values
        for key in (canonical_late_p_key(power), f"{canonical_late_p_key(power)}_reversed")
    )
    support_keys = (
        validate_gico_support_schedule_keys(_parse_csv(str(args.support_schedule_keys)))
        if str(args.support_schedule_keys).strip()
        else validate_gico_support_schedule_keys(reference_clock_keys(extra_late_p_values))
    )
    observed_keys = {str(row["scheduler_key"]) for row in rows}
    omitted_requested_keys = sorted(set(requested_extra_late_p_keys) - set(support_keys))
    if omitted_requested_keys:
        raise ValueError(
            "--extra_late_p_values requires each analytic clock and its reversed counterpart in "
            f"--support_schedule_keys; missing {omitted_requested_keys}."
        )
    missing_support_rows = sorted(set(support_keys) - observed_keys)
    if missing_support_rows:
        default_note = (
            " Omitted --support_schedule_keys selects the canonical reference-clock pool; "
            "pass an explicit reduced pool only for a deliberate reduced-pool run."
            if not str(args.support_schedule_keys).strip()
            else ""
        )
        raise ValueError(
            f"Supervision schedules must have measured context rows; missing rows for {missing_support_rows}."
            f"{default_note}"
        )
    ...
    unseen_target_nfes = sorted(
        set(
            _parse_int_csv_or_default(
                getattr(args, "unseen_target_nfe_values", ",".join(str(value) for value in CANONICAL_UNSEEN_NFES)),
                CANONICAL_UNSEEN_NFES,
            )
        )
    )
    ...
    _validate_train_tuning_reward_provenance_metadata(
        ...
        label="GICO training rows_csv",
    )
    explicit_teacher_weights = (
        _parse_float_mapping(str(args.teacher_utility_weights)) if str(args.teacher_utility_weights).strip() else None
    )
    ...
    if len(missing_metric_columns) == len(teacher_metric_target_keys):
        raise ValueError(f"GICO rows are missing all teacher metric target columns: {missing_metric_columns}")
    ...
    if sample_count > CANONICAL_CONTEXT_SAMPLE_COUNT:
        raise ValueError(
            "--context_sample_count must not exceed canonical GICO supervision cap "
    ...
    if bad_weights:
        raise ValueError(
            "weighted_normalized_regret requires J_CDN weights context=0.25,density_family=0.25,unseen_nfe=0.50."
        )
    ...
    if str(args.teacher_unseen_selection_rows_csv).strip():
        ...
        bad_unseen_source_phases = sorted(
            {_source_split_phase(row) for row in unseen_raw_rows if _source_split_phase(row) != "train_tuning"}
        )
        ...
        if _needs_forecast_uniform_rewards(unseen_filtered_rows, teacher_metric_target_keys):
            unseen_rewarded_rows = attach_uniform_gico_rewards(
                unseen_filtered_rows,
                support_schedule_keys=support_keys,
                utility_crps_weight=float(args.teacher_utility_crps_weight),
                utility_mase_weight=float(args.teacher_utility_mase_weight),
        ...
        if len(missing_unseen_columns) == len(teacher_metric_target_keys):
            raise ValueError(
                f"Teacher unseen selection rows are missing all metric target columns: {missing_unseen_columns}"
            )
    ...
    if (
        missing_density_holdout_keys
        and tuple(density_holdout_requested) == DEFAULT_DENSITY_FAMILY_HOLDOUT_SCHEDULE_KEYS
    ):
    ...
    if not selector_fit_rows:
        raise ValueError(
            "Teacher selection fitting requires at least one row after context, series, and density-family holdouts."
        )
    ...
    if unseen_selection_rows:
        unseen_embeddings_path = str(args.teacher_unseen_selection_context_embeddings_npz).strip() or str(
            args.context_embeddings_npz
        )
    ...
    student_teacher_score_weight = float(
        getattr(args, "student_teacher_score_weight", DEFAULT_STUDENT_TEACHER_SCORE_WEIGHT)
    )
    ...
    student_target_elite_fraction = float(
        getattr(args, "student_target_elite_fraction", DEFAULT_STUDENT_TARGET_ELITE_FRACTION)
    )
    student_target_elite_k = int(getattr(args, "student_target_elite_k", DEFAULT_STUDENT_TARGET_ELITE_K))
    student_target_elite_min_count = int(
        getattr(args, "student_target_elite_min_count", DEFAULT_STUDENT_TARGET_ELITE_MIN_COUNT)
    )
    ...
    if str(args.student_unseen_target_rows_csv).strip():
        ...
        _validate_unique_schedule_rows(raw_unseen_target_rows, label="Student unseen target distillation rows")
        _validate_context_embedding_checkpoint_scope(
            raw_unseen_target_rows, label="Student unseen target distillation rows"
        )
        _validate_train_tuning_rows(raw_unseen_target_rows, label="Student unseen target distillation")
        unseen_filtered_rows = _rows_for_target_nfes(raw_unseen_target_rows, unseen_target_nfes)
        if not unseen_filtered_rows:
            raise ValueError(
        ...
        unseen_target_support_keys = _observed_support(unseen_filtered_rows)
        ...
        if missing_unseen_support or extra_unseen_support:
            raise ValueError(
                "Student unseen target distillation rows must contain the same support schedule universe as seen rows; "
                f"missing={missing_unseen_support}, extra={extra_unseen_support}."
            )
        if _needs_forecast_uniform_rewards(unseen_filtered_rows, teacher_metric_target_keys):
            unseen_target_source_rows = attach_uniform_gico_rewards(
                unseen_filtered_rows,
                support_schedule_keys=support_keys,
                utility_crps_weight=float(args.teacher_utility_crps_weight),
                utility_mase_weight=float(args.teacher_utility_mase_weight),
            ...
            unseen_target_source_rows = [dict(row) for row in unseen_filtered_rows]
        ...
        unseen_embeddings_path = str(args.student_unseen_target_context_embeddings_npz).strip() or str(
            args.context_embeddings_npz
        )
        ...
        _assert_embedding_overlap_compatible(
            context_embeddings, unseen_required_embeddings, label="student_unseen_targets"
        )
        unseen_target_embeddings = embedding_normalizer.transform_table(unseen_required_embeddings)
    ...
    if student_selection_mode == STUDENT_CHECKPOINT_SELECTION_VALIDATION_CE:
        ...
        if not student_selector_fit_rows or not student_selector_validation_rows:
            raise ValueError(
                "student validation CE checkpoint selection requires non-empty selector fit and validation rows."
            )
    ...
    def _build_teacher_instance(seed_offset: int = 0, model_config: Mapping[str, Any] | None = None):
        ...
        return build_gico_teacher_model(
    ...
    def _build_student_instance(seed_offset: int = 0):
        seed_all(int(args.seed) + int(seed_offset))
        return build_gico_student_model(
    ...
    normalizer_fit_scopes = {
        "protocol": GICO_PREPROCESSING_PROTOCOL,
        "selector": {
    ...
    teacher_metric_target_coverage = {
        "protocol": GICO_TEACHER_TARGET_COVERAGE_PROTOCOL,
        "thresholds": {
    ...
    gico_protocol_metadata = {
        "protocol_revision": "gico_checkpoint_aware_cells_physical_context_holdout_v1",
        "preprocessing_protocol": GICO_PREPROCESSING_PROTOCOL,
        "teacher_target_coverage_protocol": GICO_TEACHER_TARGET_COVERAGE_PROTOCOL,
        "teacher_selection_weight_protocol": GICO_TEACHER_SELECTION_WEIGHT_PROTOCOL,
    ...
    summary_base: Dict[str, Any] = {
        ...
        "gico_protocol_metadata": gico_protocol_metadata,
        ...
        "student_unseen_target_distillation": {
            ...
            "schedule_summary_json": _artifact_input_summary(str(args.student_unseen_target_schedule_summary_json)),
            "source_row_count": int(len(unseen_target_source_rows)),
            "selected_row_count": int(len(unseen_target_rows)),
            "selected_context_count": int(len({context_id_from_row(row) for row in unseen_target_rows})),
            "selected_checkpoint_context_count": int(len({_checkpoint_context_key(row) for row in unseen_target_rows})),
        ...
        "unseen_nfe_selection": {
            ...
            "context_embeddings_npz": _artifact_input_summary(
                str(args.teacher_unseen_selection_context_embeddings_npz or args.context_embeddings_npz)
            ),
            ...
            "selected_checkpoint_context_count": int(
                len({_checkpoint_context_key(row) for row in unseen_selection_rows})
            ),
        ...
        "split_counts": {
            ...
            "student_unseen_targets": _split_counts(unseen_target_rows),
        ...
        "split_membership": {
            ...
            "student_unseen_targets": _split_membership_summary(unseen_target_rows),
    ...
    if not density_family_diagnostic_rows:
        raise ValueError(
            "weighted_normalized_regret selection requires non-empty density_family_holdout diagnostic rows."
        )
    ...
    teacher = _build_teacher_instance(0)
    final_teacher_training = train_gico_teacher(
        teacher,
        fit_rows,
    ...
    student_final_retrain = {
        "enabled": True,
        "performed": True,
        "protocol": "gico_student_final_retrain",
        ...
        "selector_validation_context_count": int(
            len({context_id_from_row(row) for row in student_selector_validation_rows})
        ),
    ...
    teacher_path = out_dir / "gico_teacher.pt"
    student_path = out_dir / "gico_student.pt"
    torch.save(
        {
            "protocol": GICO_PROTOCOL,
            "model_payload_version": MODEL_PAYLOAD_VERSION,
            ...
            "teacher_metric_target_coverage": teacher_metric_target_coverage,
            "teacher_selection_nominal_axis_weights": dict(teacher_selection_weight_metadata["nominal_axis_weights"]),
            "teacher_selection_effective_axis_weights": dict(
    ...
    summary = {
        **summary_base,
        "status": "completed",
        "policy_id": policy_id,
        "gico_teacher_checkpoint": teacher_path.name,
        "gico_student_checkpoint": student_path.name,
        "teacher_training": teacher_training,
        "teacher_checkpoint_selection": teacher_training.get("teacher_checkpoint_selection", {}),
        "student_checkpoint_selection": student_training.get("student_checkpoint_selection", {}),
        "student_final_retrain": student_training.get("student_final_retrain", {}),
    ...
    (out_dir / "gico_training_summary.json").write_text(json.dumps(summary, indent=2, sort_keys=True), encoding="utf-8")
    return summary


def main(argv: Sequence[str] | None = None) -> None:
    args = build_argparser().parse_args(argv)
    summary = train_gico(args)
