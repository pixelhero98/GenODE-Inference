...
def _context_identity_fingerprint(row: Mapping[str, Any]) -> Tuple[Tuple[str, str], ...] | None:
    ...
    if not context_schema:
        raise ValueError("Context identity fields require an explicit context_schema.")
...
def build_gico_support_preflight_report(
    rows: Sequence[Mapping[str, Any]],
    support_schedule_keys: Sequence[str],
    *,
    ...
    support_keys = _validate_gico_support_schedule_keys(support_schedule_keys)
    ...
    return {
        "artifact": "gico_rows_preflight_report",
        "schema_version": "genode_gico_support_preflight_v1",
...
def validate_gico_support_preflight_report(report: Mapping[str, Any], *, label: str = "GICO rows") -> None:
    ...
    if (
        ...
        raise ValueError(
            f"{label} failed GICO row preflight: "
...
def build_argparser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        default="auto",
        help="Comma-separated teacher utility columns, or auto for the same family defaults as genode-train-gico.",
    )
    parser.add_argument(
...
def preflight_gico_rows(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    support_keys = (
        _validate_gico_support_schedule_keys(_parse_csv(str(args.support_schedule_keys)))
    ...
    if min_context_count < 1:
        ...
        context_count_errors.append(
            ...
            f"at least {min_context_count} are required for holdout-based GICO training."
        )
