...
ROW_RECORD_FIELDS: tuple[str, ...] = (
    ...
    "molecule_energy_score",
    "molecule_feature_map_json",
...
def _runner_output_path(out_root: Path, value: Any, *, default: str, label: str) -> Path:
    return resolve_portable_relative_path(out_root, str(value or default), label=label, reject_links=True)
...
def _validate_recorder_artifact_paths(
    *,
    ...
    suffix_checks = [(jsonl_path, ".jsonl", "row JSONL name"), (csv_path, ".csv", "row CSV name")]
    ...
    input_output_collisions = {
        str(path.resolve(strict=False)): {"outputs": names_by_path[path.resolve(strict=False)], "inputs": [str(name)]}
    ...
    if output_collisions or input_output_collisions:
        raise ValueError(
            f"Evaluation artifact paths must be pairwise distinct, including reserved outputs and implicit sidecars, and inputs may not collide with outputs: output_collisions={output_collisions}, input/output_collisions={input_output_collisions}."
        )
...
def _runner_input_artifact_paths(cli_args: argparse.Namespace) -> dict[str, Path]:
    ...
    molecule_root = resolve_project_path(str(getattr(cli_args, "molecule_group_root", default_molecule_group_root())))
    for scenario_key in parse_molecule_datasets(str(getattr(cli_args, "molecule_datasets", ""))):
        inputs[f"molecule manifest {scenario_key}"] = default_molecule_group_manifest_path(scenario_key, molecule_root)
    return inputs
...
def _resolve_recorder_artifact_paths(out_root: Path, cli_args: argparse.Namespace) -> dict[str, Path]:
    paths = {
        "jsonl": _runner_output_path(
            out_root, getattr(cli_args, "row_jsonl_name", "rows.jsonl"), default="rows.jsonl", label="row JSONL name"
        ),
        "csv": _runner_output_path(
            out_root, getattr(cli_args, "row_csv_name", "rows.csv"), default="rows.csv", label="row CSV name"
        ),
        "context_csv": _runner_output_path(
            out_root, _context_row_csv_name(cli_args), default="context_rows.csv", label="context row CSV name"
        ),
...
def _load_schedule_summary_cases(path_text: str) -> list[dict[str, Any]]:
    ...
    for schedule in schedule_items:
        ...
        for item in list(schedule.get("predictions", []) or []):
            ...
            checkpoint_step = item.get(
                "checkpoint_step", schedule.get("checkpoint_step", payload.get("checkpoint_step", ""))
            )
            ...
            cases.append(
                {
                    ...
                    "reference_time_alignment": str(
                        item.get(
                            "reference_time_alignment",
                            schedule.get("reference_time_alignment", "summary_density_time_grid"),
                        )
                    ),
...
def _target_nfe_values_for_args(cli_args: argparse.Namespace) -> list[int]:
    ...
    if unknown:
        raise ValueError(
            f"target_nfe_values {unknown} are not canonical for nfe_role={role!r}; allowed values are {list(canonical)}."
        )
    if not values:
...
def _checkpoint_steps_for_args(cli_args: argparse.Namespace) -> list[int]:
    ...
    allowed = {int(value) for value in CANONICAL_CHECKPOINT_STEPS}
...
def _path_fingerprint(path: str | Path) -> dict[str, Any]:
    ...
    if payload["kind"] == "missing":
        payload["missing_location_sha256"] = hashlib.sha256(str(resolved).encode()).hexdigest()
...
def _data_path_fingerprints(cli_args: argparse.Namespace) -> dict[str, Any]:
    ...
    return {
        ...
        "molecule_manifests": {
            scenario_key: _path_fingerprint(default_molecule_group_manifest_path(scenario_key, molecule_root))
            for scenario_key in parse_molecule_datasets(str(getattr(cli_args, "molecule_datasets", "")))
        },
    }
...
def _context_reward_protocol_payload(cli_args: argparse.Namespace) -> dict[str, Any]:
    return {"version": CONTEXT_REWARD_PROTOCOL_VERSION}
...
def _choose_stratified_train_tuning_positions(
    total: int, *, fraction: float, seed: int, strata: int, dataset: str, salt: str, max_examples: int | None = None
) -> tuple[list[int], int]:
...
def _cap_context_indices(
    indices: Sequence[int], *, cap: int, seed: int, salt: str, uncapped_candidate_examples: int | None = None
    ...
    if uncapped_count < len(candidate):
        raise ValueError(
            f"uncapped_candidate_examples cannot be smaller than the selected candidate list ({uncapped_count} < {len(candidate)})."
        )
...
def _cap_context_index_groups(
    groups: Sequence[Mapping[str, Any]], *, cap: int, seed: int, salt: str
    ...
    return (
        [np.asarray(indices, dtype=np.int64) for indices in selected_by_group],
        records,
        {
...
def _protocol_config_fingerprint(cli_args: argparse.Namespace) -> str:
    payload = {
        ...
        "molecule_sample_count": int(getattr(cli_args, "molecule_sample_count", 16)),
...
def _load_rows_with_duplicate_report(
    jsonl_path: Path, *, protocol_hash: str
...
def _init_row_recorder(out_root: Path, cli_args: argparse.Namespace) -> dict[str, Any]:
    ...
    can_resume = (
        bool(getattr(cli_args, "resume", True)) and str(previous_config.get("protocol_hash", "")) == protocol_hash
    )
    ...
    existing_context_embeddings = (
        load_context_embedding_table(context_embeddings_path) if can_resume and context_embeddings_path.exists() else {}
    )
    if can_resume and _write_context_rows_enabled(cli_args):
        rows_by_key = {
            ...
            if _row_has_complete_context_artifacts(
                row, context_rows_by_signature=context_rows_by_signature, context_embeddings=existing_context_embeddings
            )
        }
    if can_resume:
    ...
    *,
    ...
    for key, value in context_embeddings.items():
        ...
        if key_text in existing_embeddings:
            ...
            if old_vec.shape != new_vec.shape or not np.allclose(old_vec, new_vec, rtol=1e-05, atol=1e-06):
    ...
    row_recorder["context_embedding_metadata"] = {
        "coverage": sorted(
            row_recorder["context_embedding_coverage"].values(),
...
def _scheduler_case_matches_command(
    case: Mapping[str, Any], *, solver_key: str, target_nfe: int, checkpoint_step: int
) -> bool:
    case_solver = str(case.get("solver_key", "") or "")
    if case_solver and case_solver != str(solver_key):
        return False
    ...
    if case_target_nfe not in ("", None) and int(case_target_nfe) != int(target_nfe):
        return False
    case_checkpoint_step = case.get("checkpoint_step", "")
    return not (case_checkpoint_step not in ("", None) and int(case_checkpoint_step) != int(checkpoint_step))
    ...
    if forecast_datasets:
        ...
        for dataset in forecast_datasets:
            add_identities_for_dataset(
                benchmark_family=FORECAST_FAMILY, dataset=str(dataset), scheduler_cases=cases_by_dataset[str(dataset)]
            )
    molecule_datasets = parse_molecule_datasets(str(getattr(cli_args, "molecule_datasets", "")))
    if molecule_datasets:
        cases_by_dataset = _scheduler_cases_for_datasets(
            cli_args, list(molecule_datasets), include_summary_cases=summary_requested
        )
...
def _schedule_context_outputs_complete(
    *, out_root: Path, cli_args: argparse.Namespace, complete_rows: Sequence[Mapping[str, Any]], protocol_hash: str
) -> tuple[bool, str]:
...
def schedule_row_output_status(out_root: Path, cli_args: argparse.Namespace) -> dict[str, Any]:
    ...
    if int(duplicate_report.get("duplicate_extra_count", 0)) > 0:
        return {
            "complete": False,
            "reason": f"duplicate rows.jsonl row keys: {duplicate_report['duplicate_extra_count']} extra rows across {duplicate_report['duplicate_key_count']} keys",
    ...
    *,
    ...
    for metric_row in per_context_metrics:
        ...
        if not context_embedding_id:
            raise ValueError("Molecule metric rows must contain a checkpoint-scoped context_embedding_id.")
        ...
        rows.append(
            {
                ...
                "axis_time_bin": _time_bin_for_target(
                    int(metric_row.get("target_t", 0)), [int(row.get("target_t", 0)) for row in per_context_metrics]
                ),
                ...
                "context_embedding_id": context_embedding_id,
                "checkpoint_id": str(checkpoint["checkpoint_id"]),
                "molecule_energy_score": metric_row.get("molecule_energy_score"),
                "molecule_feature_map_json": metric_row["molecule_feature_map_json"],
                ...
                "num_eval_samples": int(metric_row["num_eval_samples"]),
                "eval_horizon": int(rollout_steps),
                "batch_size": "",
                "sample_seed_start": int(metric_row["sample_seed_start"]),
                "sample_seed_values_json": metric_row["sample_seed_values_json"],
...
def _fixed_schedule_details(scheduler_key: str, runtime_nfe: int) -> dict[str, Any]:
    ...
    # runtime_nfe is the number of macro steps here, regardless of the outer
    # solver's number of field evaluations per step.
    mass = reference_densities("euler", int(runtime_nfe))[str(scheduler_key)]
    fixed_grid = materialize(mass, "euler", int(runtime_nfe))
...
def _schedule_details_from_case(case: Mapping[str, Any], runtime_nfe: int) -> dict[str, Any]:
    ...
    details: dict[str, Any] = {
        ...
        "realized_nfe": int(
            case.get("realized_nfe", _realized_nfe_for_solver(str(case.get("solver_key", "euler")), int(runtime_nfe)))
        ),
...
def _build_row(
    *,
    ...
    return {
        ...
        "molecule_energy_score": metrics.get("molecule_energy_score"),
        "molecule_feature_map_json": metrics.get("molecule_feature_map_json"),
        ...
        "time_grid_json": json.dumps(details["time_grid"]),
        "density_mass_json": json.dumps(details.get("density_mass")),
        "protocol_hash": str(protocol_hash),
        "row_status": "complete",
        "train_tuning_fraction": "",
        "train_tuning_seed": "",
...
def _scheduler_cases_for_datasets(
    cli_args: argparse.Namespace, datasets: Iterable[str], *, include_summary_cases: bool = True
    ...
    summary_cases = (
        _load_schedule_summary_cases(str(getattr(cli_args, "schedule_summary_json", "")))
        if include_summary_cases
        else []
    )
    ...
    if requested_summary_names and include_summary_cases:
        summary_cases = [
            case for case in summary_cases if str(case.get("scheduler_key")) in set(requested_summary_names)
        ]
        ...
        for checkpoint_step in checkpoint_steps:
            ...
            for seed in seeds:
                if str(split_phase) == TRAIN_TUNING_PHASE:
                    ...
                    tuning_seed = int(cli_args.train_tuning_seed) + int(seed) + 1000 * dataset_idx
                    ...
                    tuning_seed = int(seed) + 1000 * dataset_idx
                    eval_examples = (
                        int(selected_examples_cap) if selected_examples_cap is not None else int(len(eval_ds))
                    )
                    candidate_examples = choose_forecast_example_indices(
                        eval_ds, n_examples=int(eval_examples), seed=tuning_seed
                    )
                ...
                selection_groups.append(
                    {
                        ...
                        "selection_record": {"seed": int(seed), "tuning_seed": int(tuning_seed)},
                    }
                )
            selected_groups, selection_records, _global_selection_meta = _cap_context_index_groups(
                ...
                seed=int(cli_args.train_tuning_seed) + 1000 * dataset_idx + int(checkpoint_step),
                ...
                for target_idx, target_nfe in enumerate(target_nfes):
                    for solver_idx, solver_key in enumerate(normalize_solver_keys(str(cli_args.solver_names))):
                        ...
                        for case in pending_cases:
                            ...
                            eval_seed = int(seed) + 100000 * dataset_idx + 1000 * target_idx + solver_idx
                            ...
                            if scheduler_key != UNIFORM_SCHEDULER_KEY and cell_uniform_metrics is not None:
                                ...
                                metrics["forecast_relative_crps_gain_vs_uniform"] = _safe_relative_gain(
                                    metrics.get("forecast_crps"), cell_uniform_metrics.get("forecast_crps")
                                )
                                metrics["forecast_relative_mase_gain_vs_uniform"] = _safe_relative_gain(
                                    metrics.get("forecast_mase"), cell_uniform_metrics.get("forecast_mase")
                                )
                            row = _build_row(
                                benchmark_family=FORECAST_FAMILY,
                                split_phase=str(split_phase),
                                ...
                                row.update(
                                    {
                                        ...
                                        "train_tuning_seed": int(tuning_seed),
                                        "train_tuning_strata": int(cli_args.train_tuning_strata),
                                        "train_tuning_sampler": train_tuning_sampler_key(
                                            str(cli_args.train_tuning_sampling_mode)
                                        ),
                                        "train_tuning_sampling_mode": str(cli_args.train_tuning_sampling_mode),
                                        "train_tuning_reference_examples": int(train_tuning_reference_examples),
                                        "train_tuning_target_examples": int(
                                            selection_meta["uncapped_candidate_examples"]
                                        ),
                                        "train_tuning_uncapped_candidate_examples": int(
                                            selection_meta["uncapped_candidate_examples"]
                                        ),
                                        "train_tuning_train_split_fraction": float(
                                            cli_args.train_tuning_train_split_fraction
                                        ),
                                        "train_tuning_val_split_fraction": float(
                                            cli_args.train_tuning_val_split_fraction
                                        ),
                                    }
                                )
                                    ...
                                    copied_detail.update(
                                        {
                                            ...
                                            "nfe_role": nfe_role,
                                            "checkpoint_step": int(checkpoint_step),
                                            "checkpoint_maturity_label": _checkpoint_maturity_label(
                                                int(checkpoint_step)
                                            ),
                                            "checkpoint_maturity_index": _checkpoint_maturity_index(
                                                int(checkpoint_step)
                                            ),
                                            "effective_train_steps": int(
                                                checkpoint.get("effective_train_steps", checkpoint_step)
                                            ),
                                            "checkpoint_export_protocol": str(
                                                checkpoint.get("checkpoint_export_protocol", "")
                                            ),
    ...
    for dataset_idx, dataset in enumerate(datasets):
        ...
        for checkpoint_step in checkpoint_steps:
                ...
                checkpoint = {
                    **artifact,
                    ...
                    "train_budget_label": str(
                        artifact.get("train_budget_label", _checkpoint_maturity_label(int(checkpoint_step)))
                    ),
                }
                ...
                for seed in seeds:
                    if str(split_phase) == TRAIN_TUNING_PHASE:
                        if selected_examples_cap is None:
                            raise RuntimeError("Train-tuning example selection is missing its required cap.")
                        selection_seed = (
                            int(cli_args.train_tuning_seed) + int(seed) + 10000 * dataset_idx + 1000 * member_idx
                        )
                        ...
                        selection_seed = int(seed) + 10000 * dataset_idx + 1000 * member_idx
                        ...
                        candidate_indices = _choose_molecule_indices(
                            ds, count=int(eval_count), seed=int(selection_seed)
                        )
            ...
            selected_groups, selection_records, _global_selection_meta = _cap_context_index_groups(
                selection_groups,
                cap=_selection_cap_for_groups(selection_groups, selected_examples_cap),
                seed=10000 * int(dataset_idx) + int(checkpoint_step),
                salt=f"molecule|{dataset}|{split_phase}|{checkpoint_step}",
            )
            for selected_indices, selection_meta, work_item in zip(
                selected_groups, selection_records, work_items, strict=False
            ):
                    ...
                    for solver_idx, solver_key in enumerate(normalize_solver_keys(str(cli_args.solver_names))):
                        ...
                        for case in pending_cases:
                            ...
                            metrics = evaluate_molecule_rollout_schedule(
                                ...
                                time_grid=details["time_grid"],
                                example_indices=indices,
                                sample_count=int(getattr(cli_args, "molecule_sample_count", 16)),
                                rollout_steps=int(getattr(cli_args, "molecule_rollout_steps", 16)),
                                seed=int(eval_seed),
                                split_phase=str(split_phase),
                                checkpoint_id=str(checkpoint["checkpoint_id"]),
                            ...
                            row_metrics.update(
                                {
                                    "molecule_feature_map_json": json.dumps(
                                        metrics["metric_metadata"]["molecule_energy_score"]["feature_map"],
                                        sort_keys=True,
                                    ),
                                    "num_eval_samples": int(getattr(cli_args, "molecule_sample_count", 16)),
...
def _aggregate_seed_rows(rows: Sequence[Mapping[str, Any]]) -> list[dict[str, Any]]:
    ...
    for key, group in sorted(groups.items(), key=lambda item: tuple(str(x) for x in item[0])):
        ...
        scale_kinds = sorted(
            {
                str(row.get("forecast_mase_scale_kind"))
                for row in group
                if row.get("forecast_mase_scale_kind") not in (None, "")
            }
        )
        summary["forecast_mase_scale_kind"] = (
            scale_kinds[0] if len(scale_kinds) == 1 else ",".join(scale_kinds) if scale_kinds else ""
        )
...
def build_argparser() -> argparse.ArgumentParser:
    ...
    ap.add_argument("--molecule_sample_count", type=int, default=16)
...
def run_diffusion_flow_time_reparameterization(cli_args: argparse.Namespace) -> dict[str, Any]:
    ...
    if bool(getattr(cli_args, "diagnose_locked_forecast_only", False)):
        rows = list(_load_rows(artifact_paths["jsonl"], protocol_hash=_protocol_config_fingerprint(cli_args)).values())
    ...
    if forecast_datasets:
        scheduler_cases.update(
            _scheduler_cases_for_datasets(cli_args, list(forecast_datasets), include_summary_cases=True)
        )
    if molecule_datasets:
        scheduler_cases.update(
            _scheduler_cases_for_datasets(cli_args, list(molecule_datasets), include_summary_cases=summary_requested)
        )
    try:
        if forecast_datasets:
            _run_forecast_phase(
    ...
    selected_seed_set = {int(seed) for seed in selected_seeds}
    phase_rows = [
        row
