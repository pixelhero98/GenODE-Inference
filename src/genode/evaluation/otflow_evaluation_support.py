...
from genode.data.experiment_common import DATASET_PLANS, build_dataset_splits
...
RETIRED_BASELINE_MODEL_CONFIG_KEYS = {
    "baseline_latent_dim",
    "vae_kl_weight",
...
def resolve_reference_macro_steps(
    ...
    *,
    ...
    return max(32, int(round(factor * int(macro_steps))))
    ...
    *,
    ...
    return find_backbone_artifact(
        ...
        train_steps=int(
            ...
            else getattr(cli_args, "checkpoint_step", DEFAULT_CONDITIONAL_GENERATION_TRAIN_STEPS)
        ),
        status="ready",
    )
...
def _dataset_data_path(cli_args: argparse.Namespace, dataset: str) -> str:
    ...
    if str(dataset) == "lobster_synthetic":
        return str(getattr(cli_args, "lobster_synthetic_profile_path", "") or lobster_synthetic_profile_path())
    if str(dataset) == LONG_TERM_ST_DATASET_KEY:
        return str(getattr(cli_args, "long_term_st_path", "") or long_term_st_data_path())
    raise ValueError(f"Unknown conditional-generation dataset: {dataset}")


def resolved_checkpoint_step(cli_args: argparse.Namespace) -> int:
...
def resolved_rollout_mode(cli_args: argparse.Namespace, dataset: str) -> str:
    raw = str(getattr(cli_args, "rollout_mode", TEMPORAL_ROLLOUT_MODE) or TEMPORAL_ROLLOUT_MODE)
...
def resolved_validation_windows(cli_args: argparse.Namespace, dataset: str) -> int:
    raw = int(cli_args.eval_windows_val)
    if raw > 0:
        return raw
...
def build_conditional_generation_dataset_args(
    cli_args: argparse.Namespace,
    dataset: str,
) -> argparse.Namespace:
    plan = DATASET_PLANS[str(dataset)]
...
def load_checkpoint_model(ckpt_path: Path, device: torch.device) -> Tuple[OTFlow, Any]:
    ...
    for section_name, cls in section_types.items():
        ...
        if section_name == "model":
            ...
            if retired_keys:
                raise ValueError(
                    "Checkpoint model configuration contains retired baseline keys: "
                    f"{retired_keys}. Regenerate the checkpoint with the current OTFlow schema."
                )
...
def _validate_exact_budget_export(
    *,
    ...
    try:
        ...
        raise RuntimeError(
            f"Checkpoint {display_project_path(checkpoint_path)} has invalid exact-budget metadata."
        ) from exc

    mismatched = {key: value for key, value in metadata_steps.items() if value != expected}
    if mismatched or metadata_protocol != "exact_budget_step_state":
        raise RuntimeError(
            f"Checkpoint {display_project_path(checkpoint_path)} is not an exact-budget artifact: "
            f"expected_step={expected}, metadata={metadata_steps}, "
            f"checkpoint_export_protocol={metadata_protocol!r}."
        )

    if manifest_artifact is not None:
        for key in (*integer_fields, "checkpoint_export_protocol"):
            manifest_value = manifest_artifact.get(key)
            if manifest_value in (None, ""):
                continue
            expected_value: Any = metadata_steps[key] if key in metadata_steps else metadata_protocol
            try:
            ...
            if observed_value != expected_value:
                raise RuntimeError(
                    "Backbone manifest and checkpoint metadata disagree for "
                    f"{display_project_path(checkpoint_path)}: "
                    f"{key}={observed_value!r} != {expected_value!r}."
                )
    return (
        int(metadata_steps["checkpoint_budget_steps"]),
        int(metadata_steps["effective_train_steps"]),
        metadata_protocol,
    )
...
def _validate_forecast_checkpoint_task(
    *,
    ...
    if str(checkpoint_rollout_mode).strip().lower() != TEMPORAL_ROLLOUT_MODE:
        raise RuntimeError(
            f"Forecast checkpoint {ckpt_path} rollout_mode mismatch for {dataset}: "
            f"checkpoint={str(checkpoint_rollout_mode)!r}, expected {TEMPORAL_ROLLOUT_MODE!r}."
        )
...
def load_forecast_checkpoint_splits(
    *,
    ...
    expected_train_steps = int(resolved_checkpoint_step(cli_args))
    resolved_eval_horizon(cli_args, str(dataset))
    resolved_future_block_len(cli_args, str(dataset))
    resolved_rollout_mode(cli_args, str(dataset))
    ...
    if manifest_artifact is not None:
        ...
        resolved_checkpoint_steps = int(metadata.get("train_steps", expected_train_steps))
    ...
    checkpoint_budget_steps, effective_train_steps, checkpoint_export_protocol = _validate_exact_budget_export(
        checkpoint_path=ckpt_path,
        expected_step=expected_train_steps,
        metadata=metadata,
    ...
    return {
        ...
        "checkpoint_step": int(resolved_checkpoint_steps),
...
def load_conditional_generation_checkpoint_splits(
    *,
    ...
    manifest_artifact = _resolved_manifest_artifact(
        ...
        benchmark_family=CONDITIONAL_GENERATION_FAMILY,
        dataset_key=str(dataset),
        checkpoint_step=int(expected_train_steps),
    )
    if manifest_artifact is not None:
        ckpt_path = _resolve_checkpoint_path(str(manifest_artifact["checkpoint_path"]))
        checkpoint_id = str(manifest_artifact["checkpoint_id"])
    ...
    dataset_args = build_conditional_generation_dataset_args(cli_args, str(dataset))
            ...
            for chunk_start in chunk_starts:
                ...
                if return_per_example_rows or return_context_embeddings:
                    ...
                    for metadata in metadata_rows:
                        chunk_context_ids.append(
                            stable_context_id(
                                scenario_key=str(metadata["scenario_key"]),
                ...
                for row_idx, true_block_raw in enumerate(true_blocks):
                    ...
                    if return_per_example_rows:
                        ...
                        context_index = int(chunk_start) + int(row_idx)
                        context_evaluation_seed = int(evaluation_seed) + int(context_index)
                        sample_seed_values = [
                            int(context_evaluation_seed) + 1_000_000 * int(sample_idx)
                            for sample_idx in range(int(num_eval_samples))
                        ]
