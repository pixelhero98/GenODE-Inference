...
PIPELINE_VERSION = "canonical_multi_family_gico_pipeline"
DEFAULT_STAGES = (
    "data_prep",
    "backbone_training",
    "ser_summaries",
...
GICO_ABLATION_STUDENT_STAGE = "gico_ablation_students"
...
DEFAULT_ABLATION_FIRST_STAGES = (
    ...
    GICO_ABLATION_LOCKED_TEST_STAGE,
)
PIPELINE_STAGE_ORDER = (*DEFAULT_STAGES, GICO_ABLATION_STUDENT_STAGE, GICO_ABLATION_LOCKED_TEST_STAGE)
DEFAULT_GICO_TEACHER_STEPS = 500
DEFAULT_GICO_STUDENT_STEPS = 500
...
def _requested_schedule_keys(args: argparse.Namespace) -> List[str]:
    ...
    for power in extra_powers:
        base_key = canonical_late_p_key(power)
        requested_set.update((base_key, f"{base_key}_reversed"))
    canonical_order = reference_clock_keys(extra_powers)
    ordered = [key for key in canonical_order if key in requested_set]
    ordered.extend(key for key in requested if key not in set(canonical_order) and key not in set(ordered))
    return ordered
...
def _display_run_root(run_root: Path) -> str:
    display = _display_path(run_root)
    if not display.startswith("external/"):
        return display
    parts = run_root.parts
    for marker in ("full_pipeline", "gico_ablations"):
        if marker in parts:
...
def _display_command(command: Sequence[str], *, run_root: Path | None = None) -> List[str]:
    ...
    for token in command:
        ...
        if path.is_absolute():
            if run_root is not None and path.is_relative_to(run_root):
                out.append(_run_relative_path(run_root, path))
            else:
...
def _display_stage(entry: StageCommand, *, run_root: Path | None = None) -> Dict[str, Any]:
    return {
        ...
        "commands": [_display_command(command, run_root=run_root) for command in entry.commands],
    }
...
def _student_objective_args(args: argparse.Namespace, override: GicoAblationArm | None = None) -> List[Any]:
    ...
    if bool(source.student_teacher_score_include_unseen_targets):
...
def _protocol_payload(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    return {
        ...
        "student_teacher_score_include_unseen_targets": bool(args.student_teacher_score_include_unseen_targets),
...
def _has_ablation_stage(commands: Sequence[StageCommand]) -> bool:
    ablation_stages = {GICO_ABLATION_STUDENT_STAGE, GICO_ABLATION_LOCKED_TEST_STAGE}
    return any(entry.stage in ablation_stages for entry in commands)


def _ablation_root(run_root: Path, preset: str) -> Path:
    if run_root.name == str(preset) and run_root.parent.name == "gico_ablations":
        return run_root
    return run_root / "gico_ablations" / str(preset)
...
def _resolve_run_root(args: argparse.Namespace) -> Path:
    ...
    if bool(getattr(args, "ablation_first", False)):
        ...
        return scenario_root / "gico_ablations" / preset
...
def _build_ablation_manifest(
    ...
    *,
    ...
    arms = []
    for arm in gico_ablation_arms(preset):
        train_root = root / arm.arm_id / "gico" / arm.student_training_mode
        report_root = root / arm.arm_id / "locked_test_reports" / arm.student_training_mode
        arms.append(
            {
                **arm.manifest_record(),
                "outputs": {
                    "training_summary": _run_relative_path(run_root, train_root / "gico_training_summary.json"),
                    "student_checkpoint": _run_relative_path(run_root, train_root / "gico_student.pt"),
                    "locked_test_report_root": _run_relative_path(run_root, report_root),
                },
            }
        )
    manifest = {
        "artifact": "gico_ablation_manifest",
        "schema_version": "genode_gico_ablation_v1",
        "status": status,
        ...
        "gico_teacher_steps": int(args.gico_teacher_steps),
        "gico_student_steps": int(args.gico_student_steps),
        "schedule_keys": _requested_schedule_keys(args),
        "arms": arms,
...
def _stage_outputs_complete(entry: StageCommand) -> Tuple[bool, str]:
    for command in entry.commands:
        ...
        ok, reason = _command_path_exists(
            ...
            module="genode.gico.train_gico",
            out_arg="--out_dir",
            relative_path="gico_student.pt",
        )
        if not ok:
            return False, reason
        ok, reason = _command_json_output_exists(
            command,
            module="genode.gico.report_locked_test",
            out_arg="--out_dir",
            relative_path="locked_test_gico_policy_summary.json",
        )
        if not ok:
            return False, reason
        for relative_path in (
            "locked_test_gico_rows.csv",
            "locked_test_gico_aggregate_rows.csv",
            "locked_test_gico_decisions.csv",
        ):
            ok, reason = _command_path_exists(
                command,
                module="genode.gico.report_locked_test",
...
def _stage_manifest_complete(run_root: Path, entry: StageCommand, *, protocol_hash: str) -> Tuple[bool, str]:
    ...
    if not isinstance(manifest_hashes, list):
        return False, "stage manifest does not contain command hashes"
...
def _build_stage_commands(args: argparse.Namespace, run_root: Path) -> List[StageCommand]:
    ...
    ablation_arms = gico_ablation_arms(ablation_preset)
    ...
    def _gico_train_command(mode: str, out_dir: Path, arm: GicoAblationArm | None = None) -> List[str]:
        command_args: List[Any] = [
            ...
            support_schedules,
            "--extra_late_p_values",
            str(getattr(args, "extra_late_p_values", "")),
            "--teacher_steps",
            int(args.gico_teacher_steps),
            "--student_steps",
            int(args.gico_student_steps),
        ...
        *,
        ...
        for role, nfe_values in role_nfes.items():
            for checkpoint_step in checkpoint_values:
                ...
                commands.append(
                    _python_module_command(
                        "genode.gico.report_locked_test",
                        [
                            "--gico_student_checkpoint",
                            student_root / "gico_student.pt",
                            "--training_summary",
                            student_root / "gico_training_summary.json",
                            "--context_rows",
    ...
    def _gico_ablation_student_commands() -> List[List[str]]:
        return [
            _gico_train_command(
                ...
                gico_ablation_root / arm.arm_id / "gico" / arm.student_training_mode,
                arm,
            )
    ...
    def _gico_ablation_locked_test_commands() -> List[List[str]]:
        ...
        for arm in ablation_arms:
            commands.extend(
                _locked_test_report_commands_for_student(
                    student_root=gico_ablation_root / arm.arm_id / "gico" / arm.student_training_mode,
                    report_root=gico_ablation_root / arm.arm_id / "locked_test_reports" / arm.student_training_mode,
                )
            )
        return commands
    ...
    commands = [
        ...
        StageCommand(
            GICO_ABLATION_STUDENT_STAGE,
            _gico_ablation_student_commands(),
            "gico_ablation_students_manifest.json",
        ),
        StageCommand(
            GICO_ABLATION_LOCKED_TEST_STAGE,
            _gico_ablation_locked_test_commands(),
            "gico_ablation_locked_test_reports_manifest.json",
        ),
    ]
...
def run_full_pipeline(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    _write_json(
        _status_path(run_root),
        {
            ...
            "protocol_hash": protocol_hash,
            "run_root": _display_run_root(run_root),
        ...
        if not bool(args.dry_run):
            for command_idx, command in enumerate(entry.commands):
                if command and command[0] == "internal":
                    raise RuntimeError(
                        f"Unresolved internal pipeline command in non-dry-run: "
                        f"{_display_command(command, run_root=run_root)}"
                    )
                ...
                if int(result.returncode) != 0:
                    ...
                    if has_ablation_stage:
                        ...
                        _write_json(
                            _ablation_root(run_root, str(args.gico_ablation_preset)) / "ablation_manifest.json",
    ...
    summary = {
        ...
        "stages": [_display_stage(entry, run_root=run_root) for entry in commands],
...
def build_argparser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--extra_late_p_values",
        default="",
        help="Comma-separated analytic late-p supervision values in [1.5, 8]; base and reversed clocks are added.",
    )
    parser.add_argument(
