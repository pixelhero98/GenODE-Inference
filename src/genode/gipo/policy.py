...
def validate_conditioning_style(
...
def normalize_teacher_utility_weights(
    ...
    if unknown:
        raise ValueError(f"teacher utility weights contain unknown keys: {unknown}; expected {list(keys)}.")
    ...
    for key in keys:
        if key in raw:
            ...
            value = float(reference_weights.get(key, 1.0))
...
def teacher_utility_weights_for_summary(target_keys: Sequence[str], weights: Mapping[str, float] | None) -> Dict[str, float]:
    return {
        str(key): float(value)
        for key, value in normalize_teacher_utility_weights(target_keys, weights).items()
    }
...
def _resolve_setting_encoder_config(
    ...
    if setting_encoder_config is None:
        return setting_encoder_config_from_payload({"mode": validate_setting_mode(mode)})
    return setting_encoder_config_from_payload(setting_encoder_config)


def _json_hash(payload: Mapping[str, Any], *, prefix: str) -> str:
    return schedule_json_hash(payload, prefix=prefix)
...
def context_embedding_id_from_row(row: MetricRow) -> str:
    """Return the stable key for looking up frozen context embeddings.
...
def split_rows_by_density_family_holdout(
    ...
    *,
    ...
    metadata = {
        ...
        "reward_anchor_scheduler_key": UNIFORM_SCHEDULE_KEY,
    }
    ...
    for key, group in sorted(grouped.items(), key=lambda item: item[0]):
        ...
        for row in group:
            reward_columns = uniform_anchored_objective_columns(
                ...
                uniform_rows[0],
                FORECAST_METRIC_SPECS,
                uniform_scheduler_key=uniform_key,
                eps=float(eps),
            )
            ...
            copied.update(
                {
                    ...
                    "reward_anchor_scheduler_key": uniform_key,
                    **reward_columns,
...
def recommended_context_calibration_count(
    ...
    *,
    ...
    cap = int(max_total)
    requested = int(reference_total) if normalized_combined_reference is None else int(round(0.20 * float(normalized_combined_reference)))
...
def sample_context_ids_stratified(
    ...
    *,
    ...
    for context_id, row in by_context.items():
        ...
        strata[(scenario_key, series, stratum, formula, trajectory, time_bin)].append(context_id)
    rng = np.random.default_rng(int(seed))
    selected: List[str] = []
...
def _row_reward_metric_weights(row: MetricRow, target_keys: Sequence[str]) -> Dict[str, float]:
    ...
    keys = validate_teacher_metric_target_keys(target_keys)
    unknown = sorted(set(str(key) for key in payload) - set(keys))
    if unknown:
        raise ValueError(
            f"reward_metric_weights_json contains unknown keys: {unknown}; expected a subset of {list(keys)}."
        )
    out: Dict[str, float] = {}
    for key in keys:
        if key in payload:
    ...
    base.update(
        {
            ...
            **{
                f"metric_huber_loss_{key}": (None if value is None else float(value))
...
def read_metric_rows_csv(path: str | Path) -> List[Dict[str, Any]]:
    ...
    for row_index, row in enumerate(rows):
        reject_retired_evaluation_keys(row, source=f"Evaluation row {row_index} from {Path(path).name}")
