...
def _validate_unique_schedule_rows(rows: Sequence[Mapping[str, Any]], *, label: str) -> None:
    ...
    if duplicates:
        ...
        raise ValueError(
            ...
            "(scenario_key,solver_key,target_nfe,context_id,logical_seed,checkpoint_scope,scheduler_key) cells; "
    ...
    *,
    ...
    for row_idx, row in enumerate(rows):
        ...
        if present_target_keys:
            ...
            anchor = str(row.get("reward_anchor_scheduler_key", "")).strip()
            if anchor != UNIFORM_SCHEDULE_KEY:
                raise ValueError(
                    f"{label} row {row_idx} has utility targets but reward_anchor_scheduler_key={anchor!r}; "
        ...
        if _has_nonempty_value(row, "train_tuning_fraction"):
            ...
            if not math.isfinite(fraction) or abs(fraction - TRAIN_TUNING_FRACTION) > 1e-12:
                raise ValueError(
                    f"{label} row {row_idx} train_tuning_fraction must be {TRAIN_TUNING_FRACTION:.2f}; "
                    f"found {fraction!r}."
                )
...
def train_gipo(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    requested_setting_mode = SETTING_ENCODER_MODE_CONTINUOUS
    mode = validate_setting_mode(requested_setting_mode)
    ...
    if missing_density_holdout_keys and tuple(density_holdout_requested) == DEFAULT_DENSITY_FAMILY_HOLDOUT_SCHEDULE_KEYS:
        supported_holdout_keys = [key for key in density_holdout_requested if key in support_set]
        if not supported_holdout_keys:
            raise ValueError(
        ...
        density_holdout_keys: List[str] = supported_holdout_keys
    ...
    setting_encoder_config = setting_encoder_config_for_rows(final_fit_rows, mode=mode)
    ...
    gipo_protocol_metadata = {
        "protocol_revision": "gipo_checkpoint_aware_cells_physical_context_holdout",
