...
def _clean_path_token(value: str | None, *, label: str, default: str | None = None) -> str:
    ...
    if token in {".", ".."} or not re.fullmatch(r"[A-Za-z0-9_.-]+", token):
...
def _molecule_source_archive_name(value: Any) -> str:
    name = str(value or "")
    relative = portable_relative_path(name, label="Molecule group manifest source_zip_name")
    if len(relative.parts) != 1:
        raise ValueError("Molecule group manifest source_zip_name must be a file name, not a path.")
    return name


def _molecule_processed_dir(value: Any) -> PurePosixPath:
...
def load_molecule_group_manifest(dataset_key: str, group_root: str | Path | None = None) -> Dict[str, Any]:
    ...
    for idx, row in enumerate(rows):
        ...
        _clean_path_token(str(row.get("stratum", "")), label="stratum")
    source_zip_names = payload.get("source_zip_names", [])
    if not isinstance(source_zip_names, list):
        raise ValueError("Molecule group manifest source_zip_names must be a list.")
    for source_zip_name in source_zip_names:
        _molecule_source_archive_name(source_zip_name)
    return payload
...
def trainable_molecule_group_members(manifest: Mapping[str, Any]) -> List[Dict[str, Any]]:
    ...
    for raw_member in manifest.get("strata", []):
        ...
        if not bool(member.get("trainable", True)) or bool(member.get("mixed_shape", False)):
            continue
        _molecule_source_archive_name(member.get("source_zip_name"))
        _molecule_processed_dir(member.get("processed_dir"))
        _clean_path_token(str(member.get("member_key", "")), label="member_key")
        _clean_path_token(stratum, label="stratum")
